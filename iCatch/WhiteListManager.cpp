...
int CWhiteListManager::LoadData(const WHITE_LIST_TYPE type){
    if(type & KAVE_WHITE_LIST){
        FILE* fp = fopen("./whitelist/KaveWhiteList.dat","r");
        if(fp == NULL){
            return 1;
        }
        ...
        fclose(fp);
    }else if(type & BIGDATA_WHITE_LIST){
        FILE* fp = fopen("./whitelist/BigDataBlackList.dat","r");
        if(fp == NULL){
            return 1;
        }
...
WHITELIST_RESULT CWhiteListManager::CheckMD5(UInt32 uiMD5,HASH_MAP_MD5_ITERATOR* pPos){
    if(pPos != NULL){
